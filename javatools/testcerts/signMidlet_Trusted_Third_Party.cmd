...
@ECHO ON
%JDK%bin\java.exe -jar %WTK%bin\JADTool.jar -addcert -keystore %JKEY_STORE%test_keystore.ks -alias JP_Third_Party_Trusted -storepass jperformance -certnum 1 -chainnum 1 -inputjad %JAD_NAME%  -outputjad %JAD_NAME%
%JDK%bin\java.exe -jar %WTK%bin\JADTool.jar -addjarsig -jarfile %JAR_NAME% -keystore %JKEY_STORE%test_keystore.ks -alias JP_Third_Party_Trusted -storepass jperformance -keypass R_and_D_Party_Trusted -inputjad %JAD_NAME% -outputjad %JAD_NAME%
...
ECHO sigMidlet_Third_Party_Trusted.cmd "JAD_FILE.JAD" "JAR_FILE.JAR"
