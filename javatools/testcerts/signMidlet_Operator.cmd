...
@ECHO ON
%JDK%bin\java.exe -jar %WTK%bin\JADTool.jar -addcert -keystore %JKEY_STORE%test_keystore.ks -alias JP_Operator_Domain -storepass jperformance -certnum 1 -chainnum 1 -inputjad %JAD_NAME% -outputjad %JAD_NAME%
%JDK%bin\java.exe -jar %WTK%bin\JADTool.jar -addjarsig -jarfile %JAR_NAME% -keystore %JKEY_STORE%test_keystore.ks -alias JP_Operator_Domain -storepass jperformance -keypass R_and_D_Operator_Domain -inputjad %JAD_NAME% -outputjad %JAD_NAME%
...
ECHO sigMidlet_operator.cmd "JAD_FILE.JAD" "JAR_FILE.JAR"
