...
public class ManagerImpl implements PlugIn
{
    ...
    // CMMAEventSource
    private static int sEventSourceHandle;
    ...
    private final MIDEventServer iEventServer;
    ...
    private ManagerImpl()
    {
        ...
        iEventServer = new MIDEventServer(new String("java-mma"));
        sEventSourceHandle = _createEventSource(iEventServer.getHandle());
        if (sEventSourceHandle < NativeError.KErrNone)
        {
            throw new OutOfMemoryError("SymbianOS error: " + sEventSourceHandle);
        }
        // Suite UID was fetched using deprecated MIDletSuiteAMS.getMIDletSuiteId.
        // Using hard coded value in here since MMAPI doesn't really use the
        // suite id even if it passes it to native.
        final int midletSuiteId = 1;
        sManagerHandle = _createManager(sEventSourceHandle, midletSuiteId);
        if (sManagerHandle < NativeError.KErrNone)
        {
            throw new OutOfMemoryError();
        }

        ToolkitInvoker ti = ToolkitInvoker.getToolkitInvoker();
        // register for destroy event
        ti.addObserver(ti.getToolkit(),  new ToolkitObserver()
        {
        ...
        iForegroundListener = new ForegroundListener(sEventSourceHandle);
    }
    ...
    /**
     * This will be called when object is finalized.
     */
    ...
    {
        String[] protocols = _getSupportedProtocols(sEventSourceHandle,
                             sManagerHandle,
                             aContentType);
    ...
    {
        String[] types = _getSupportedContentTypes(sEventSourceHandle,
                         sManagerHandle,
                         aProtocol);
     ...
     */
    private native void _release(int aEventSourceHandle);

    private static native String[] _getSupportedContentTypes(int aEventSourceHandle,
            int aManagerHandle,
            String aContentType);

    private static native String[] _getSupportedProtocols(int aEventSourceHandle,
            int aManagerHandle,
            String aProtocol);
}
