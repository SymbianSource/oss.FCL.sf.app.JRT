...
public abstract class Dialog {
	...
	Shell parent;
	String title;
	protected boolean iIsOpen = false;
	protected String iResultString = null;
	protected int iResult = 0;
    protected RGB	iRgb = null;
...
/**
 * Sends dialog result  to the receiver 
 * and also notifies receiver that native dialog has been closed
 * This method is invoked by dispatching dialog close event.  
 *
 * @param string the string returned by native dialog
 *
 * @exception SWTException <ul>
 *    <li>ERROR_WIDGET_DISPOSED - if the receiver has been disposed</li>
 *    <li>ERROR_THREAD_INVALID_ACCESS - if not called from the thread that created the receiver</li>
 * </ul>
 */
protected void internal_sendResult( String string ){
	iResultString = string;	
	iIsOpen = false;	
}
 ...
 * This method is invoked by dispatching dialog close event.  
 *
 * @param result the integer returned by native dialog
 *
 * @exception SWTException <ul>
 *    <li>ERROR_WIDGET_DISPOSED - if the receiver has been disposed</li>
 *    <li>ERROR_THREAD_INVALID_ACCESS - if not called from the thread that created the receiver</li>
 * </ul>
 */
protected void internal_sendResult(int result ){
	iResult = result;
	iIsOpen = false;	
}
 ...
 * </ul>
 */
protected void internal_sendResult(int r, int g, int b ){
	iRgb = null;
	if ( r != -1 && r != -2 )
	{
	iRgb =new RGB( r, g, b );
...
/**
 * Looping until the flag (iIsOpen) is set to false.
 * Events also get dispatched during looping
 */
protected void internal_waitForClose( ){
	Display display = parent.getDisplay();
	while ( iIsOpen ) {
