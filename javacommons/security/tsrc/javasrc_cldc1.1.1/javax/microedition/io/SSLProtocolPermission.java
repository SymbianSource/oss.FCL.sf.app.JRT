 ...
 * ssl://{host}:{portspec}
 * <p>
 * The exact syntax for the SSLProtocolPermission URI is provided by this BNF.
 * The value of the {host} field must be a symbolic hostname, a literal IPv4
 * address or a literal IPv6 address, as specified by RFC 3986. If the {host}
 * string is a DNS name, an asterisk may appear in the leftmost position to
 * indicate a match of 1 or more entire domain labels. Partial domain label
 * matches are not permitted, therefore "*.sun.com" is valid, but "*sun.com"
 ...
 * "ssl://*:*" matches secure connections to all hosts on all ports.
 *
 * @see Connector.open(java.lang.String)
 * @see "javax.microedition.io.SecureConnection" in MIDP 3.0 Specification
 *
...
{
    /**
     * Creates a new SSLProtocolPermission with the specified URI as its
    ...
    /**
     * Checks if this SSLProtocolPermission object "implies" the
     ...
     * two SSLProtocolPermission objects requires the underlying
     ...
     * best-effort. For each SSLProtocolPermission constructed with a
     ...
     * <ul>
     * <li>p is an instanceof SSLProtocolPermission, and</li>
     ...
     * @return true if obj is a SSLProtocolPermission and has the same
     *         URI string as this SSLProtocolPermission object.
     */
     ...
     * SSLProtocolPermission objects.
     * <p>
     * SSLProtocolPermission objects must be stored in a manner that allows
