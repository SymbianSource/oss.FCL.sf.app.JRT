...
TEST(TestRegistry, TestRegistry)
{
    ...
    TRAPD(err, entry = registry->RegistryEntryL(uid));
    ...
    TRAP(err, entry = registry->RegistryEntryL(uid));
    // CHECK(KErrNotFound == err);
    CHECK(NULL == entry);
...
TEST(TestRegistry, TestGetRegistryEntryUidsL)
{
    ...
    js->startTransaction();
    CHECK(jtu->populate(*js, APPLICATION_TABLE, populateEntry2));
    ...
    js->startTransaction();
    CHECK(jtu->populate(*js, APPLICATION_TABLE, populateEntry3));
    ...
    js->open();
    js->startTransaction();
    CHECK(jtu->populate(*js, APPLICATION_TABLE, populateEntry4));
    ...
    CHECK(jtu->remove(*js, APPLICATION_TABLE, populateEntry));
    CHECK(jtu->remove(*js, APPLICATION_TABLE, populateEntry2));
    CHECK(jtu->remove(*js, APPLICATION_TABLE, populateEntry3));
    CHECK(jtu->remove(*js, APPLICATION_TABLE, populateEntry4));
    CHECK(jtu->remove(*js, APPLICATION_PACKAGE_TABLE, suite1));
    CHECK(jtu->remove(*js, APPLICATION_PACKAGE_TABLE, suite2));
    js->close();
